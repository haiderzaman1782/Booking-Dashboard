...
/**
 * Calculate weekly data from appointments and calls
 */
export const calculateWeeklyData = (appointments = [], calls = []) => {
  ...
  return days.map((day, index) => {
    ...
    const dayCalls = calls.filter(call => {
      const callDate = normalizeDate(call.timestamp || call.callTimestamp || call.createdAt);
...
/**
 * Calculate monthly data from appointments and calls
 */
export const calculateMonthlyData = (appointments = [], calls = []) => {
  const today = new Date();
  const weeks = [];
  ...
  for (let i = 3; i >= 0; i--) {
    ...
    const weekAppointments = appointments.filter(apt => {
      const aptDateStr = normalizeDate(apt.date);
    ...
    const weekCalls = calls.filter(call => {
      const callDateStr = normalizeDate(call.timestamp || call.callTimestamp || call.createdAt);
 ...
 */
export const calculateServiceDistribution = (appointments = []) => {
  const serviceCounts = {};
  ...
  const colors = ['#3b82f6', '#14b8a6', '#8b5cf6', '#f59e0b', '#ec4899', '#10b981', '#ef4444'];
