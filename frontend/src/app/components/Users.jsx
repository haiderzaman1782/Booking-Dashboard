...
export function Users({ users, appointments, payments, callLogs }) {
  ...
  // Suspicious users (multiple failed calls or payments)
  const suspiciousUsers = usersData.filter(u => (u.failedCalls >= 3 || u.failedPayments >= 2));
  ...
  const getUserAppointments = (userId) => {
    return appointments?.filter(apt => 
      apt.patientName === usersData.find(u => u.id === userId)?.fullName
    ) || [];
  };
  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      ...
      <Card className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 p-2 sm:p-4 rounded-xl shadow-md dark:shadow-none">
        <CardHeader>
          <CardTitle className="dark:text-white">User Activity (Calls & Appointments)</CardTitle>
        </CardHeader>
        <CardContent>
          <ResponsiveContainer width="100%" height={300}>
            <BarChart data={[
              { name: "Mon", calls: 45, appointments: 32 },
              { name: "Tue", calls: 52, appointments: 38 },
              { name: "Wed", calls: 48, appointments: 35 },
              { name: "Thu", calls: 61, appointments: 42 },
              { name: "Fri", calls: 55, appointments: 40 },
              { name: "Sat", calls: 28, appointments: 18 },
              { name: "Sun", calls: 15, appointments: 10 }
            ]}>
              <CartesianGrid strokeDasharray="3 3" stroke="#e5e7eb" className="dark:stroke-gray-700" />
              <XAxis dataKey="name" stroke="#6b7280" className="dark:stroke-gray-400" />
              <YAxis stroke="#6b7280" className="dark:stroke-gray-400" />
              ...
              <div className="flex items-center gap-2">
                ...
                <Select value={sortBy} onValueChange={setSortBy}>
                  ...
                  <SelectContent className="dark:bg-gray-900 dark:border-gray-700">
                    ...
                    <SelectItem value="mostAppointments">Most Appointments</SelectItem>
                  </SelectContent>
                </Select>
              </div>
            </div>
            ...
            <Table>
              <TableHeader>
                <TableRow className="dark:border-gray-700">
                  ...
                  <TableHead className="dark:text-gray-300 whitespace-nowrap">Appointments</TableHead>
          ...
          {selectedUser && (
            <div className="space-y-6 mt-4">
              ...
              <div className="border-t border-gray-200 dark:border-gray-700 pt-4">
                ...
                {(() => {
                  const userAppointments = getUserAppointments(selectedUser.id);
                  return userAppointments.length > 0 ? (
                    <div className="space-y-2 max-h-40 overflow-y-auto">
                      {userAppointments.slice(0, 5).map((apt) => (
                        <div key={apt.id} className="p-2 bg-gray-50 dark:bg-gray-900/50 rounded-lg">
                          <div className="flex justify-between items-center">
                            <span className="text-sm dark:text-gray-300">{apt.service}</span>
                      ...
                      {userAppointments.length > 5 && (
                        <p className="text-xs text-gray-500 dark:text-gray-400 text-center">
                          +{userAppointments.length - 5} more appointments
                        </p>
                      )}
                    </div>
                  ) : (
              ...
              <div className="border-t border-gray-200 dark:border-gray-700 pt-4">
                ...
                {(() => {
                  const userCalls = getUserCalls(selectedUser.id);
                  return userCalls.length > 0 ? (
                    <div className="space-y-2 max-h-40 overflow-y-auto">
                      {userCalls.slice(0, 5).map((call) => (
                        <div key={call.id} className="p-2 bg-gray-50 dark:bg-gray-900/50 rounded-lg">
                          <div className="flex justify-between items-center">
                            <span className="text-sm dark:text-gray-300">{call.purpose}</span>
                            <Badge variant="outline" className="text-xs">
                              {call.status}
                            </Badge>
                          </div>
                          <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">
                            {formatTimestamp(call.timestamp)} • Duration: {call.duration}
                          </p>
                        </div>
                      ))}
                      {userCalls.length > 5 && (
                        <p className="text-xs text-gray-500 dark:text-gray-400 text-center">
                          +{userCalls.length - 5} more calls
                        </p>
                      )}
                    </div>
                  ) : (
                    <p className="text-sm text-gray-500 dark:text-gray-400">No call history found</p>
                  );
                })()}
              </div>
